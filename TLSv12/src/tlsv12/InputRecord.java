...
/**
 * SSL 3.0 records, as pulled off a TCP stream. Input records are basically
 * buffers tied to a particular input stream ... a layer above this must map
 * these records into the model of a continuous stream of data.
 *
 * Since this returns SSL 3.0 records, it's the layer that needs to map SSL 2.0
 * style handshake records into SSL 3.0 ones for those "old" clients that
 * interop with both V2 and V3 servers. Not as pretty as might be desired.
 *
 * NOTE: During handshaking, each message must be hashed to support verification
 * that the handshake process wasn't compromised.
 *
...
class InputRecord extends ByteArrayInputStream implements Record {
    ...
    /*
     * Construct the record to hold the maximum sized input record. Data will be
    ...
    /*
     * For handshaking, we need to be able to hash every byte above the record
     * marking layer. This is where we're guaranteed to see those bytes, so this
     * is where we can hash them ... especially in the case of hashing the
     * initial V2 message!
     */
    ...
    void decrypt(MAC signer, CipherBox box) throws BadPaddingException {
        ...
        if( tagLen != 0 ) {
            ...
            if( checkMacTags(contentType(), buf, headerSize, contentLen,
                    signer, false) ) {
                if( reservedBPE == null ) {
            ...
            if( box.isCBCMode() ) {
                int remainingLen = calculateRemainingLen(signer,
                        cipheredLength, contentLen);
     ...
     */
    static boolean checkMacTags(byte contentType, byte[] buffer, int offset,
            int contentLen, MAC signer, boolean isSimulated) {

        int tagLen = signer.MAClen();
        byte[] hash = signer.compute(contentType, buffer, offset, contentLen,
                isSimulated);
    ...
    /*
     * Calculate the length of a dummy buffer to run MAC computation and
     * comparison on the remainder.
     * 
    ...
    static int calculateRemainingLen(MAC signer, int fullLen, int usedLen) {
        ...
        return 0x01
                + (int) (Math.ceil(fullLen / (1.0d * blockLen)) - Math.ceil(usedLen
                        / (1.0d * blockLen))) * signer.hashBlockLen();
    }


    /*
     * Well ... hello_request messages are _never_ hashed since we can't know
     * when they'd appear in the sequence.
     */
     ...
     * 
     * There is one place where we want to access the hash in the middle of a
     * record: client cert message gets hashed, and part of the same record is
     * the client cert verify message which uses that hash. So we track how much
     * we've read and hashed.
     */
    void doHashes() {
    ...
    /*
     * Handshake messages may cross record boundaries. We "queue" these in big
     * buffers if we need to cope with this problem. This is not anticipated to
     * be a common case; if this turns out to be wrong, this can readily be sped
     * up.
     */
    void queueHandshake(InputRecord r) throws IOException {
        ...
        /*
         * Move any unread data to the front of the buffer, flagging it all as
         * unhashed.
         */
        if( pos > headerSize ) {
            len = count - pos;
        ...
        /*
         * Adjust lastHashed; important for now with clients which send SSL V2
         * client hellos. This will go away eventually, by buffer code cleanup.
         */
    ...
    /**
     * Prevent any more data from being read into this record, and flag the
     * record as holding no data.
     */
    public void close() {
        appDataValid = false;
        isClosed = true;
    ...
    /*
     * We may need to send this SSL v2 "No Cipher" message back, if we are faced
     * with an SSLv2 "hello" that's not saying "I talk v3". It's the only one
     * documented in the V2 spec as a fatal error.
     */
    private static final byte[] v2NoCipher = { (byte) 0x80, (byte) 0x03, // unpadded
                                                                         // 3
                                                                         // byte
                                                                         // record
    ...
    /*
     * Read the SSL V3 record ... first time around, check to see if it really
     * IS a V3 record. Handle SSL V2 clients which can talk V3.0, as well as
     * real V3 record format; otherwise report an error.
     */
    void read(InputStream s, OutputStream o) throws IOException {
        ...
        /*
         * For SSL it really _is_ an error if the other end went away so
         * ungracefully as to not shut down cleanly.
         */
        if( exlen < headerSize ) {
         ...
         * typically SSL v2 header. (PCT could also be detected here.) This case
         * is currently common -- Navigator 3.0 usually works this way, as do IE
         * 3.0 and other products.
         */
        if( !formatVerified ) {
            formatVerified = true;
            /*
             * The first record must either be a handshake record or an alert
             * message. If it's not, it is either invalid or an SSLv2 message.
             */
    ...
    private void readV3Record(InputStream s, OutputStream o) throws IOException {
        ...
        if( (recordVersion.v < ProtocolVersion.MIN.v)
            ...
            throw new SSLException("Unsupported record version "
                    + recordVersion);
        }
        ...
        if( contentLen < 0 || contentLen > maxLargeRecordSize - headerSize ) {
            throw new SSLProtocolException("Bad InputRecord size"
                    + ", count = " + contentLen + ", buf.length = "
                    + buf.length);
        }

        /*
         * Grow "buf" if needed. Since buf is maxRecordSize by default, this
         * only occurs when we receive records which violate the SSL
         * specification. This is a workaround for a Microsoft SSL bug.
         */
        ...
        if( debug != null && Debug.isOn("record") ) {
            ...
            System.out.println(Thread.currentThread().getName() + ", READ: "
                    + recordVersion + " " + contentName(contentType())
                    + ", length = " + available());
        }
        /*
    ...
    private void handleUnknownRecord(InputStream s, OutputStream o) throws IOException {
        /*
         * No? Oh well; does it look like a V2 "ClientHello"? That'd be an
         * unpadded handshake message; we don't bother checking length just now.
         */
        if( ((buf[0] & 0x080) != 0) && buf[2] == 1 ) {
            /*
             * if the user has disabled SSLv2Hello (using setEnabledProtocol)
            ...
            ProtocolVersion recordVersion = ProtocolVersion.valueOf(buf[3],
                    buf[4]);

            if( recordVersion == ProtocolVersion.SSL20Hello ) {
                /*
                 * Looks like a V2 client hello, but not one saying
                 * "let's talk SSLv3". So we send an SSLv2 error message, one
                 * that's treated as fatal by clients. (Otherwise we'll hang.)
                 */
                try {
            ...
            /*
             * If we can map this into a V3 ClientHello, read and hash the rest
             * of the V2 handshake, turn it into a V3 ClientHello message, and
             * pass it up.
             */
            ...
            if( exlen < len + headerSize ) {
                int really = readFully(s, v2Buf, exlen - headerSize, len
                        + headerSize - exlen);
                if( really < 0 ) {
                    throw new EOFException("SSL peer shut down incorrectly");
                }
            ...
            /*
             * If this is a V2 NoCipher message then this means the other server
             * doesn't support V3. Otherwise, we just don't understand what it's
             * saying.
             */
    ...
    /*
     * Actually do the write here. For SSLEngine's HS data, we'll override this
     * method and let it take the appropriate action.
     */
     ...
     * example, Navigator 3.0 clients. The V2 message is in the header and the
     * bytes passed as parameter. This routine translates the V2 message into an
     * equivalent V3 one.
     */
    private void V2toV3ClientHello(byte v2Msg[]) throws SSLException {
        ...
        /*
         * Build the first part of the V3 record header from the V2 one that's
         * now buffered up. (Lengths are fixed up later).
         */
        buf[0] = ct_handshake;
        buf[1] = buf[3]; // V3.x
        buf[2] = buf[4];
        ...
        /*
         * Copy Random value/nonce ... if less than the 32 bytes of a V3
         * "Random", right justify and zero pad to the left. Else just take the
         * last 32 bytes.
         */
         ...
         * 
         * 0/3, SSL_RSA_EXPORT_WITH_RC4_40_MD5 0/6,
        ...
        /*
         * Fill in lengths of the messages we synthesized (nested: V3 handshake
         * message within V3 record) and then return
         */
    ...
    /**
     * Return a description for the given content type. This method should be in
     * Record, but since that is an interface this is not possible. Called from
     * InputRecord and OutputRecord.
     */
