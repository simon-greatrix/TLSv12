...
/**
 * This class computes the "Message Authentication Code" (MAC) for each SSL
 * message. This is essentially a shared-secret signature, used to provide
 * integrity protection for SSL messages. The MAC is actually one of several
 * keyed hashes, as associated with the cipher suite and protocol version. (SSL
 * v3.0 uses one construct, TLS uses another.)
 * <P>
 * NOTE: MAC computation is the only place in the SSL protocol that the sequence
 * number is used. It's also reset to zero with each change of a cipher spec, so
 * this is the only place this state is needed.
 *
...
final class MAC {
     ...
     *            record type
     * @param buf
     *            compressed record on which the MAC is computed
     * @param offset
     *            start of compressed record data
     * @param len
     *            the size of the compressed record
     * @param isSimulated
     *            if true, simulate the the MAC computation
     */
     ...
     *
     * On return, the bb position == limit, and limit will have not changed.
     *
     * @param type
     *            record type
     * @param bb
     *            a ByteBuffer in which the position and limit demarcate the
     *            data to be MAC'd.
     * @param isSimulated
     *            if true, simulate the the MAC computation
     */
     ...
     * numbers do not wrap. When the sequence number is near to wrap, we need to
     * close the connection immediately.
     */
    final boolean seqNumOverflow() {
        /*
         * Conservatively, we don't allow more records to be generated when
         * there are only 2^8 sequence numbers left.
         */
        return (block != null && mac != null && block[0] == (byte) 0xFF
                && block[1] == (byte) 0xFF && block[2] == (byte) 0xFF
                && block[3] == (byte) 0xFF && block[4] == (byte) 0xFF
     ...
     * 
     * Sequence numbers are of type uint64 and may not exceed 2^64-1. Sequence
     * numbers do not wrap. If a TLS implementation would need to wrap a
     * sequence number, it must renegotiate instead.
     */
    final boolean seqNumIsHuge() {
        /*
         * Conservatively, we should ask for renegotiation when there are only
         * 2^48 sequence numbers left.
         */
        return (block != null && mac != null && block[0] == (byte) 0xFF && block[1] == (byte) 0xFF);
    }
    ...
    /*
     * Compute based on either buffer type, either bb.position/limit or
     * buf/offset/len.
     */
    private byte[] compute(byte type, ByteBuffer bb, byte[] buf, int offset,
            int len, boolean isSimulated) {
