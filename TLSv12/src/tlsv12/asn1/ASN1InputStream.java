...
public class ASN1InputStream extends FilterInputStream implements BERTags {
    ...
    /**
     * Create an ASN1InputStream based on the input byte array. The length of
     * DER objects in the stream is automatically limited to the length of the
     * input array.
     * 
     * @param input
    ...
    /**
     * Create an ASN1InputStream where no DER object will be longer than limit,
     * and constructed objects such as sequences will be parsed lazily.
     *
     * @param input
     *            stream containing ASN.1 encoded data.
     * @param limit
    ...
    protected ASN1Primitive buildObject(int tag, int tagNo, int length) throws IOException {
        ...
        if( (tag & APPLICATION) != 0 ) {
            return new DERApplicationSpecific(isConstructed, tagNo,
                    defIn.toByteArray());
        }
        ...
        if( isConstructed ) {
            // TODO There are other tags that may be constructed (e.g.
            // BIT_STRING)
            switch (tagNo) {
            case OCTET_STRING:
                //
    ...
    private static byte[] getBuffer(DefiniteLengthInputStream defIn,
            byte[][] tmpBuffers) throws IOException {
    ...
    static ASN1Primitive createPrimitiveDERObject(int tagNo,
            DefiniteLengthInputStream defIn, byte[][] tmpBuffers) throws IOException {
        switch (tagNo) {
        case BIT_STRING:
        ...
        case NULL:
            return DERNull.INSTANCE; // actual content is ignored (enforce 0
                                     // length?)
        case NUMERIC_STRING:
            return new DERNumericString(defIn.toByteArray());
        case OBJECT_IDENTIFIER:
            return ASN1ObjectIdentifier.fromOctetString(getBuffer(defIn,
                    tmpBuffers));
        case OCTET_STRING:
