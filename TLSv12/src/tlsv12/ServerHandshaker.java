...
import tlsv12.HandshakeMessage.ServerKeyExchange;
import tlsv12.SignatureAndHashAlgorithm.HashAlgorithm;
import tlsv12.SignatureAndHashAlgorithm.SignatureAlgorithm;
...
/**
 * ServerHandshaker does the protocol handshaking from the point of view of a
 * server. It is driven asychronously by handshake messages as delivered by the
...
final class ServerHandshaker extends Handshaker {
    ...
    /*
     * For anonymous and ephemeral Diffie-Hellman key exchange, we use ephemeral
     * Diffie-Hellman keys.
     */
    private DHCrypt dh;
     ...
     * authentication is required. Otherwise, we will need to wait for the next
     * handshake.
     */
    ...
    /*
     * This routine handles all the server side handshake messages, one at a
    ...
    void processMessage(byte type, int message_len) throws IOException {
        ...
        case HandshakeMessage.ht_client_key_exchange:
            ...
            case K_DH_ANON:
                 ...
                 * Diffie-Hellman calculation. Note that the main protocol
                 * difference in these five flavors is in how the
                 * ServerKeyExchange message was constructed!
                 */
                preMasterSecret = this.clientKeyExchange(new DHClientKeyExchange(
                        input));
                break;
            case K_ECDH_RSA:
            case K_ECDH_ECDSA:
            ...
            case K_ECDHE_ECDSA:
            case K_ECDH_ANON:
                preMasterSecret = this.clientKeyExchange(new ECDHClientKeyExchange(
                        input));
                break;
            default:
                throw new SSLProtocolException("Unrecognized key exchange: "
                        + keyExchange);
            }
        ...
        default:
            throw new SSLProtocolException("Illegal server handshake msg, "
                    + type);
        }
     ...
     * will use. If needed, it also writes its Certificate plus in some cases a
     * ServerKeyExchange message. It may also write a CertificateRequest, to
     * elicit a client certificate.
     * 
     * All these messages are terminated by a ServerHelloDone message. In most
     * cases, all this can be sent in a single Record.
     */
    private void clientHello(ClientHello mesg) throws IOException {
        ...
        /*
         * Always make sure this entire record has been digested before we start
         * emitting output, to ensure correct digesting order.
         */
        input.digestNow();

        /*
         * FIRST, construct the ServerHello using the options and priorities
         * from the ClientHello. Update the (pending) cipher spec as we do so,
         * and save the client's version to protect against rollback attacks.
         * 
         * There are a bunch of minor tasks here, and one major one: deciding if
         * the short or the full handshake sequence will be used.
         */
        ...
        if( session == null ) {
            ...
            session = new SSLSessionImpl(protocolVersion, CipherSuite.C_NULL,
                    getLocalSupportedSignAlgs(), sslContext.getSecureRandom(),
         ...
         * 
         * NOTE: while an "anonymous RSA" mode is explicitly allowed by the
         * protocol, we can't support it since all of the SSL flavors defined in
         * the protocol spec are explicitly stated to require using RSA
         * certificates.
         */
        if( (keyExchange != K_DH_ANON) && (keyExchange != K_ECDH_ANON) ) {
            ...
            /*
             * Set local certs in the SSLSession, output debug info, and then
             * actually write to the client.
             */
         ...
         * 
         * It's usually needed unless there's an encryption-capable RSA cert, or
         * a D-H cert. The notable exception is that exportable ciphers used
         * with big RSA keys need to downgrade to use short RSA keys, even when
         * the key/cert encrypts OK.
         */
        ...
        case K_DHE_DSS:
            try {
                m3 = new DH_ServerKeyExchange(dh, privateKey,
                        clnt_random.random_bytes, svr_random.random_bytes,
                        sslContext.getSecureRandom(),
                        preferableSignatureAlgorithm, protocolVersion);
        ...
        case K_ECDH_ANON:
            try {
                m3 = new ECDH_ServerKeyExchange(ecdh, privateKey,
                        clnt_random.random_bytes, svr_random.random_bytes,
                        sslContext.getSecureRandom(),
                        preferableSignatureAlgorithm, protocolVersion);
            } catch (GeneralSecurityException e) {
                throwSSLException("Error generating ECDH server key exchange",
                        e);
                m3 = null; // make compiler happy
            }
            break;
        ...
        if( doClientAuth != SSLEngineImpl.clauth_none
            ...
            m4 = new CertificateRequest(caCerts, keyExchange, localSignAlgs,
                    protocolVersion);
        ...
        /*
         * Flush any buffered messages so the client will see them. Ideally, all
         * the messages above go in a single network level message to the
         * client. Without big Certificate chains, it's going to be the common
         * case.
         */
        output.flush();
    }
    ...
    /*
     * Choose cipher suite from among those supported by client. Sets the
     * cipherSuite and keyExchange variables.
     */
    ...
    /**
     * Set the given CipherSuite, if possible. Return the result. The call
     * succeeds if the CipherSuite is available and we have the necessary
     * certificates to complete the handshake. We don't check if the CipherSuite
     * is actually enabled.
     *
     * If successful, this method also generates ephemeral keys if required for
     * this ciphersuite. This may take some time, so this method should only be
     * called if you really want to use the CipherSuite.
     *
     * This method is called from chooseCipherSuite() in this class.
     */
    boolean trySetCipherSuite(CipherSuite suite) {
        /*
         * If we're resuming a session we know we can support this key exchange
         * algorithm and in fact have already cached the result of it in the
         * session state.
         */
        if( resumingSession ) {
            return true;
        ...
        if( protocolVersion.v >= ProtocolVersion.TLS12.v ) {
            if( peerSupportedSignAlgs != null ) {
                ...
                if( algorithm == null ) {
                    ...
                    supportedSignAlgs = new ArrayList<SignatureAndHashAlgorithm>(
                            1);
    ...
    /*
     * Acquire some "ephemeral" Diffie-Hellman keys for this handshake. We don't
     * reuse these, for improved forward secrecy.
     */
    private void setupEphemeralDHKeys(boolean export) {
        /*
         * Diffie-Hellman keys ... we use 768 bit private keys due to the
         * "use twice as many key bits as bits you want secret" rule of thumb,
         * assuming we want the same size premaster secret with Diffie-Hellman
         * and RSA key exchanges. Except that exportable ciphers max out at 512
         * bits modulus values.
         */
    ...
    private void setupStaticECDHKeys() {
        ...
        ecdh = new ECDHCrypt((ECPrivateKey) privateKey,
                (ECPublicKey) certs[0].getPublicKey());
    }


    /**
     * Retrieve the server key and certificate for the specified algorithm from
     * the KeyManager and set the instance variables.
     *
    ...
    /*
     * Diffie Hellman key exchange is used when the server presented D-H
     * parameters in its certificate (signed using RSA or DSS/DSA), or else the
     * server presented no certificate but sent D-H params in a
     * ServerKeyExchange message. Use of D-H is specified by the cipher suite
     * chosen.
     * 
     * The message optionally contains the client's D-H public key (if it wasn't
     * not sent in a client certificate). As always with D-H, if a client and a
     * server have each other's D-H public keys and they use common algorithm
     * parameters, they have a shared key that's derived via the D-H
     * calculation. That key becomes the pre-master secret.
     */
     ...
     * authentication messages are included in the checksums used to validate
     * the handshake (e.g. Finished messages). Other than that, the _exact_
     * identity of the client is less fundamental to protocol security than its
     * role in selecting keys via the pre-master secret.
     */
    ...
    /*
     * Client writes "finished" at the end of its handshake, after cipher spec
     * is changed. We verify it and then send ours.
     * 
     * When we're resuming a session, we'll have already sent our own Finished
     * message so just the verification is needed.
     */
    private void clientFinished(Finished mesg) throws IOException {
        ...
        /*
         * Verify if client did send the certificate when client authentication
         * was required, otherwise server should not proceed
         */
        if( doClientAuth == SSLEngineImpl.clauth_required ) {
        ...
        /*
         * Verify the client's message with the "before" digest of messages, and
         * forget about continuing to use that digest.
         */
         ...
         * "Finished" message to the hash of handshake messages, then send the
         * change_cipher_spec and Finished message.
         */
        if( !resumingSession ) {
            input.digestNow();
        ...
        if( !resumingSession && session.isRejoinable() ) {
            ...
            System.out.println("%% Didn't cache non-resumable server session: "
                    + session);
        }
    }
    ...
    /*
     * Compute finished message with the "server" digest (and then forget about
     * that digest, it can't be used again).
     */
    private void sendChangeCipherAndFinish(boolean finishedTag) throws IOException {
         ...
         * message will be the last handshake message. Flush, and now we are
        ...
        /*
         * Update state machine so client MUST send 'finished' next The update
         * should only take place if it is not in the fast handshake mode since
         * the server has to wait for a finished message from the client.
         */
        if( finishedTag ) {
    ...
    void handshakeAlert(byte description) throws SSLProtocolException {
        ...
        /*
         * It's ok to get a no_certificate alert from a client of which we
         * *requested* authentication information. However, if we *required* it,
         * then this is not acceptable.
         * 
         * Anyone calling getPeerCertificates() on the session will get an
    ...
    /*
     * Verify the certificate sent by the client. We'll only get one if we sent
     * a CertificateRequest to request client authentication. If we are in TLS
     * mode, the client may send a message with no certificates to indicate it
     * does not have an appropriate chain. (In SSLv3 mode, it would send a no
     * certificate alert).
     */
    private void clientCertificate(CertificateMsg mesg) throws IOException {
        ...
        if( peerCerts.length == 0 ) {
            /*
             * If the client authentication is only *REQUESTED* (e.g. not
             * *REQUIRED*, this is an acceptable condition.)
             */
        ...
        try {
            ...
            tm.checkClientTrusted(peerCerts.clone(), authType);
