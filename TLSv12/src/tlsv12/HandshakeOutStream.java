...
/**
 * Output stream for handshake data. This is used only internally to the SSL
 * classes.
 *
 * MT note: one thread at a time is presumed be writing handshake messages, but
 * (after initial connection setup) it's possible to have other threads
 * reading/writing application data. It's the SSLSocketImpl class that
 * synchronizes record writes.
 *
...
public class HandshakeOutStream extends OutputStream {
    ...
    /*
     * Update the handshake data hashes ... mostly for use after a client cert
     * has been sent, so the cert verify message can be constructed correctly
     * yet without forcing extra I/O. In all other cases, automatic hash
     ...
     * Hashes are updated automatically if something gets flushed to the network
     * (e.g. a big cert message etc).
     */
    ...
    public void flush() throws IOException {
        if( socket != null ) {
            ...
            /*
             * Even if record might be empty, flush anyway in case there is a
             * finished handshake message that we need to queue.
             */
    ...
    /*
     * Tell the OutputRecord that a finished message was contained either in
     * this record or the one immeiately preceeding it. We need to reliably pass
     * back notifications that a finish message occured.
     */
    void setFinishedMsg() {
        assert (socket == null);
    ...
    /*
     * Put integers encoded in standard 8, 16, 24, and 32 bit big endian
     * formats. Note that OutputStream.write(int) only writes the least
     * significant 8 bits and ignores the rest.
     */
    ...
    /*
     * Put byte arrays with length encoded as 8, 16, 24 bit integers in
     * big-endian format.
     */
    ...
    private void checkOverflow(int length, int overflow) {
        if( length >= overflow ) {
            ...
            throw new RuntimeException(
                    "Field length overflow, the field length (" + length
