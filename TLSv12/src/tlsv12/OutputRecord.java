 ...
 * application. It may grow/shrink due to compression and will be modified in
 * place for mac-ing and encryption.
 *
 * Handshake records have additional needs, notably accumulation of a set of
 * hashes which are used to establish that handshaking was done right. Handshake
 * records usually have several handshake messages each, and we need
 * message-level control over what's hashed.
 *
...
class OutputRecord extends ByteArrayOutputStream implements Record {
    ...
    /*
     * Default constructor makes a record supporting the maximum SSL record
     * size. It allocates the header bytes directly.
     * 
    ...
    /**
     * Get the size of the buffer we need for records of the specified type.
     */
    ...
    /*
     * Reset the record so that it can be refilled, starting immediately after
     * the header.
     */
    ...
    /*
     * We hash (the plaintext) on demand. There is one place where we want to
     * access the hash in the middle of a record: client cert message gets
     * hashed, and part of the same record is the client cert verify message
     * which uses that hash. So we track how much of each record we've hashed so
     * far.
     */
    void doHashes() {
    ...
    private void hashInternal(byte buf[], int offset, int len) {
        if( debug != null && Debug.isOn("data") ) {
            try {
                ...
                hd.encodeBuffer(new ByteArrayInputStream(buf, lastHashed, len),
                        System.out);
            } catch (IOException e) {}
        }
    ...
    /*
     * Return true iff the record is empty -- to avoid doing the work of sending
     * empty records over the network.
     */
    boolean isEmpty() {
    ...
    /*
     * Compute the MAC and append it to this record. In case we are
     * automatically flushing a handshake stream, make sure we have hashed the
     * message first.
     */
    void addMAC(MAC signer) throws IOException {
        ...
        if( signer.MAClen() != 0 ) {
            byte[] hash = signer.compute(contentType, buf, headerSize, count
                    - headerSize, false);
            write(hash);
        }
    }
     ...
     * 
     * Note that the increased capacity is only can be used for held record
     * buffer. Please DO NOT update the availableDataBytes() according to the
     * expended buffer capacity.
     * 
    ...
    /*
     * Write the record out on the stream. Note that you must have (in order)
     * compressed the data, appended the MAC, and encrypted it in order for the
     * record to be understood by the other end. (Some of those steps will be
     * null early in handshaking.)
     * 
     * Note that this does no locking for the connection, it's required that
     * synchronization be done elsewhere. Also, this does its work in a single
     * low level write, for efficiency.
     */
    void write(OutputStream s, boolean holdRecord,
            ByteArrayOutputStream heldRecordBuffer) throws IOException {
        /*
         * Don't emit content-free records. (Even change cipher spec messages
         * have a byte of data!)
         */
        if( count == headerSize ) {
            return;
        ...
        /*
         * If this is the initial ClientHello on this connection and we're not
         * trying to resume a (V3) session then send a V2 ClientHello instead so
         * we can detect V2 servers cleanly.
         */
        if( firstMessage && useV2Hello() ) {
            ...
            if( debug != null && Debug.isOn("record") ) {
                System.out.println(Thread.currentThread().getName()
                        + ", WRITE: SSLv2 client hello message" + ", length = "
        ...
        /*
         * The upper levels may want us to delay sending this packet so multiple
         * TLS Records can be sent in one (or more) TCP packets. If so, add this
         * packet to the heldRecordBuffer.
         * 
        ...
        if( holdRecord ) {
             ...
             * 
             * Don't worry about the override of writeBuffer(), because when
             * holdRecord is true, the implementation in this class will be
             * used.
             */
            ...
            if( heldRecordBuffer != null && heldRecordBuffer.size() > 0 ) {
                ...
                System.arraycopy(heldRecordBuffer.toByteArray(), 0, buf, 0,
                        heldLen);
                count += heldLen;
    ...
    void writeBuffer(OutputStream s, byte[] buf, int off, int len,
        ...
        if( debug != null && Debug.isOn("packet") ) {
            try {
                HexDumpEncoder hd = new HexDumpEncoder();
                ByteBuffer bb = ByteBuffer.wrap(buf, off + debugOffset, len
                        - debugOffset);
    ...
    /*
     * Return whether the buffer contains a ClientHello message that should be
    ...
    private boolean useV2Hello() {
        return firstMessage && (helloVersion == ProtocolVersion.SSL20Hello)
                ...
                && (buf[headerSize + 4 + 2 + 32] == 0); // V3 session ID is
                                                        // empty
    }
     ...
     * example, Netscape Commerce 1.0 servers. The V3 message is in the header
     * and the bytes passed as parameter. This routine translates the V3 message
     * into an equivalent V2 one.
     * 
     * Note that the translation will strip off all hello extensions as SSL V2.0
     * does not support hello extension.
     */
    private void V3toV2ClientHello(byte v3Msg[]) throws SSLException {
         ...
         * Therefore, just copy over the V3 cipher spec values with a leading 0.
         */
        ...
        for(int i = 0;i < cipherSpecs;i++) {
            ...
            if( !containsRenegoInfoSCSV && byte1 == (byte) 0x00
                    && byte2 == (byte) 0xFF ) {
     ...
     */
    private static int[] V3toV2CipherMap1 = { -1, -1, -1, 0x02, 0x01, -1, 0x04,
            0x05, -1, 0x06, 0x07 };

    private static int[] V3toV2CipherMap3 = { -1, -1, -1, 0x80, 0x80, -1, 0x80,
            0x80, -1, 0x40, 0xC0 };


    /*
     * See which matching pure-V2 cipher specs we need to include. We are
     * including these not because we are actually prepared to talk V2 but
     * because the Oracle Web Server insists on receiving at least 1 "pure V2"
     * cipher suite that it supports and returns an illegal_parameter alert
     * unless one is present. Rather than mindlessly claiming to implement all
     * documented pure V2 cipher suites the code below just claims to implement
     * the V2 cipher suite that is "equivalent" in terms of cipher algorithm &
     * exportability with the actual V3 cipher suite that we do support.
     */
