...
import static tlsv12.CipherSuite.KeyExchange.K_DH_ANON;
import static tlsv12.CipherSuite.KeyExchange.K_ECDH_ANON;
import static tlsv12.CipherSuite.KeyExchange.K_RSA;
import static tlsv12.CipherSuite.KeyExchange.K_RSA_EXPORT;

import tlsv12.HandshakeMessage.CertificateMsg;
import tlsv12.HandshakeMessage.CertificateRequest;
import tlsv12.HandshakeMessage.CertificateVerify;
import tlsv12.HandshakeMessage.ClientHello;
import tlsv12.HandshakeMessage.DH_ServerKeyExchange;
import tlsv12.HandshakeMessage.ECDH_ServerKeyExchange;
import tlsv12.HandshakeMessage.Finished;
import tlsv12.HandshakeMessage.HelloRequest;
import tlsv12.HandshakeMessage.ServerHello;
import tlsv12.HandshakeMessage.ServerHelloDone;
 ...
 * client. It is driven asychronously by handshake messages as delivered by the
 * parent Handshaker class, and also uses common functionality (e.g. key
 * generation) that is provided there.
 *
 * @author David Brownell
 */
final class ClientHandshaker extends Handshaker {

    /** Pattern to match IPv4 literal addresses */
    private static final Pattern PAT_IP_LIT = Pattern.compile("^(?:[0-9A-Fa-f]{0,4}:)*(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$");
    ...
    /*
     * The RSA PreMasterSecret needs to know the version of ClientHello that was
     * used on this handshake. This represents the "max version" this client is
     * supporting. In the case of an initial handshake, it's the max version
     * enabled, but in the case of a resumption attempt, it's the version of the
     * session we're trying to resume.
     */
    private ProtocolVersion maxProtocolVersion;

    // To switch off the SNI extension.
    private final static boolean enableSNIExtension = Debug.getBooleanProperty(
     ...
     * guarantee that the server certificate change in the renegotiation is
     * legal. However, endpoint identification is only enabled for HTTPS and
     * LDAP over SSL/TLS by default. It is not enough to protect SSL/TLS
     * connections other than HTTPS and LDAP.
     * 
     * The renegotiation indication extension (See RFC 5764) is a pretty strong
     * guarantee that the endpoints on both client and server sides are
     * identical on the same connection. However, the Triple Handshake attacks
     * can bypass this guarantee if there is a session-resumption handshake
     * between the initial full handshake and the renegotiation full handshake.
     * 
     * Server certificate change may be unsafe and should be restricted if
     * endpoint identification is not enabled and the previous handshake is a
     * session-resumption abbreviated initial handshake, unless the identities
     * reproesented by both certificates can be regraded as the same (See
     ...
     * server certificate change in renegotiation is allowed or not. The default
     * value of the system property is "false". To mitigate the compatibility
     * impact, applications may want to set the system property to "true" at
     * their own risk.
     * 
     * If the value of the system property is "false", server certificate change
     * in renegotiation after a session-resumption abbreviated initial handshake
     * is restricted (See isIdentityEuivalent()).
     * 
     ...
     */
    private final static boolean allowUnsafeServerCertChange = Debug.getBooleanProperty(
            "jdk.tls.allowUnsafeServerCertChange", false);

    /*
     * the reserved server certificate chain in previous handshaking
     * 
     * The server certificate chain is only reserved if the previous handshake
     * is a session-resumption abbreviated initial handshake.
     */
    ...
    ClientHandshaker(SSLSocketImpl socket, SSLContextImpl context,
            ...
            boolean secureRenegotiation, byte[] clientVerifyData,
    ...
    /*
     * This routine handles all the client side handshake messages, one at a
     * time. Given the message type (and in some cases the pending cipher spec)
     * it parses the type-specific message. Then it calls a function that
     * handles that specific message.
     * 
     * It updates the state machine (need to verify it) as each message is
     * processed, and writes responses as needed using the connection in the
     * constructor.
     */
    void processMessage(byte type, int messageLen) throws IOException {
        ...
        case HandshakeMessage.ht_server_key_exchange:
            ...
            case K_DHE_RSA:
                try {
                    this.serverKeyExchange(new DH_ServerKeyExchange(input,
                            serverKey, clnt_random.random_bytes,
                            svr_random.random_bytes, messageLen,
            ...
            case K_ECDH_ANON:
                try {
                    this.serverKeyExchange(new ECDH_ServerKeyExchange(input,
                            serverKey, clnt_random.random_bytes,
                            svr_random.random_bytes, localSupportedSignAlgs,
                            protocolVersion));
                } catch (GeneralSecurityException e) {
                    throwSSLException("Server key", e);
                }
            ...
            default:
                throw new SSLProtocolException(
                        "unsupported key exchange algorithm = " + keyExchange);
            }
            break;
        ...
        default:
            throw new SSLProtocolException("Illegal client handshake msg, "
                    + type);
        }
     ...
     * "client hello" to renegotiate current cipher specs (e.g. maybe lots of
     * data has been encrypted with the same keys, or the server needs the
    ...
    /*
     * Server chooses session parameters given options created by the client --
     * basically, cipher options, session id, and someday a set of compression
     * options.
     * 
     * There are two branches of the state machine, decided by the details of
     * this message. One is the "fast" handshake, where we can resume the
     * pre-existing session we asked resume. The other is a more expensive
     * "full" handshake, with key exchange and probably authentication getting
     * done.
     */
    private void serverHello(ServerHello mesg) throws IOException {
        ...
        if( serverHelloRI != null ) {
            if( isInitialHandshake ) {
                ...
                byte[] verifyData = new byte[clientVerifyData.length
                        + serverVerifyData.length];
                System.arraycopy(clientVerifyData, 0, verifyData, 0,
        ...
        session = new SSLSessionImpl(protocolVersion, cipherSuite,
                getLocalSupportedSignAlgs(), mesg.sessionId, getHostSE(),
                getPortSE());
        setHandshakeSessionSE(session);
        if( debug != null && Debug.isOn("handshake") ) {
    ...
    /*
     * Diffie-Hellman key exchange. We save the server public key and our own
     * D-H algorithm object so we can defer key calculations until after we've
     * sent the client key exchange message (which gives client and server some
     * useful parallelism).
     */
    ...
    /*
     * The server's "Hello Done" message is the client's sign that it's time to
    ...
    private void serverHelloDone(ServerHelloDone mesg) throws IOException {
        ...
        /*
         * Always make sure the input has been digested before we start emitting
         * data, to ensure the hashes are correctly computed for the Finished
         * and CertificateVerify messages which we send (here).
         */
        input.digestNow();

        /*
         * FIRST ... if requested, send an appropriate Certificate chain to
         * authenticate the client, and remember the associated private key to
         * sign the CertificateVerify message.
         */
        ...
        /*
         * SECOND ... send the client key exchange message. The procedure used
         * is a function of the cipher suite selected; one is always needed.
         */
        HandshakeMessage m2;
        ...
        case K_DH_DSS:
            /*
             * For DH Key exchange, we only need to make sure the server knows
             * our public key, so we calculate the same pre-master secret.
             * 
             * For certs that had DH keys in them, we send an empty handshake
             * message (no key) ... we flag this case by passing a null
             * "dhPublic" value.
             * 
        ...
        default:
            ...
            throw new RuntimeException("Unsupported key exchange: "
                    + keyExchange);
        }
        if( debug != null && Debug.isOn("handshake") ) {
            m2.print(System.out);
        ...
        /*
         * THIRD, send a "change_cipher_spec" record followed by the "Finished"
         * message. We flush the messages we've queued up, to get concurrency
         * between client and server. The concurrency is useful as we calculate
         * the master secret, which is needed both to compute the "Finished"
         * message, and to compute the keys used to protect all records
         * following the change_cipher_spec.
         */
         ...
         * keying data; we do it now. Deferring this calculation is good from a
         * performance point of view, since it lets us do it during some time
         * that network delays and the server's own calculations would otherwise
         * cause to be "dead" in the critical path.
         */
         ...
         * CertificateVerify (unless the key in the client's certificate was a
         * Diffie-Hellman key).).
         * 
         * This uses a hash of the previous handshake messages ... either a
         * nonfinal one (if the particular implementation supports it) or else
         * using the third element in the arrays of hashes being computed.
         */
        if( signingKey != null ) {
            ...
            try {
                ...
                if( protocolVersion.v >= ProtocolVersion.TLS12.v ) {
                    preferableSignatureAlgorithm = SignatureAndHashAlgorithm.getPreferableAlgorithm(
    ...
    /*
     * "Finished" is the last handshake message sent. If we got this far, the
     * MAC has been validated post-decryption. We validate the two hashes here
     * as an additional sanity check, protecting the handshake against various
     * active attacks.
     */
    private void serverFinished(Finished mesg) throws IOException {
         ...
         * "Finished" message to the hash of handshake messages, then send our
         * own change_cipher_spec and Finished message for the server to verify
         * in turn. These are the last handshake messages.
         * 
         * In any case, update the session cache. We're done handshaking, so
         * there are no threats any more associated with partially completed
         * handshakes.
         */
        if( resumingSession ) {
            input.digestNow();
        ...
        if( !resumingSession ) {
            if( session.isRejoinable() ) {
                ...
                System.out.println("%% Didn't cache non-resumable client session: "
                        + session);
            }
        }
    }
    ...
    /*
     * Send my change-cipher-spec and Finished message ... done as the last
     * handshake act in either the short or long sequences. In the short one,
     * we've already seen the server's Finished; in the long one, we wait for it
     * now.
     */
    private void sendChangeCipherAndFinish(boolean finishedTag) throws IOException {
        ...
        /*
         * Send the change_cipher_spec message, then the Finished message which
         * we just calculated (and protected using the keys we just calculated).
         * Server responds with its Finished message, except in the
         * "fast handshake" (resume session) case.
         */
        ...
        /*
         * Update state machine so server MUST send 'finished' next. (In "long"
         * handshake case; in short case, we're responding to its message.)
         */
        state = HandshakeMessage.ht_finished - 1;
    }
    ...
    HandshakeMessage getKickstartMessage() throws SSLException {
        ...
        //
        session = ((SSLSessionContextImpl) sslContext.engineGetClientSessionContext()).get(
                getHostSE(), getPortSE());
        if( debug != null && Debug.isOn("session") ) {
            if( session != null ) {
                System.out.println("%% Client cached " + session
        ...
        if( session != null ) {
            ...
            if( !allowUnsafeServerCertChange && session.isSessionResumption() ) {
                try {
                    ...
                    reservedServerCerts = (X509Certificate[]) session.getPeerCertificates();
        ...
        if( session != null ) {
            ...
            /*
             * Force use of the previous session ciphersuite, and add the SCSV
             * if enabled.
             */
            if( !enableNewSession ) {
                if( session == null ) {
                ...
                Collection<CipherSuite> cipherList = new ArrayList<CipherSuite>(
                        2);
        ...
        ClientHello clientHelloMessage = new ClientHello(
                sslContext.getSecureRandom(), maxProtocolVersion, sessionId,
                cipherSuites);
        ...
        if( enableSNIExtension ) {
            ...
            if( hostname != null && hostname.indexOf('.') > 0
                    && !PAT_IP_LIT.matcher(hostname).matches() ) {
        ...
        /*
         * need to set the renegotiation_info extension for: 1: secure
         * renegotiation 2: initial handshake and no SCSV in the ClientHello 3:
         * insecure renegotiation and no SCSV in the ClientHello
         */
        if( secureRenegotiation || !cipherSuites.contains(CipherSuite.C_SCSV) ) {
    ...
    /*
     * Unless we are using an anonymous ciphersuite, the server always sends a
     * certificate message (for the CipherSuites we currently support). The
     * trust manager verifies the chain for us.
     */
    private void serverCertificate(CertificateMsg mesg) throws IOException {
        ...
        try {
            ...
            tm.checkServerTrusted(peerCerts.clone(), keyExchangeString);
     ...
     * 
     * The certificates can be used to represent the same identity: 1. If the
     * subject alternative names of IP address are present in both certificates,
     * they should be identical; otherwise, 2. if the subject alternative names
     * of DNS name are present in both certificates, they should be identical;
     * otherwise, 3. if the subject fields are present in both certificates, the
    ...
    private static boolean isIdentityEquivalent(X509Certificate thisCert,
        ...
        if( thisIPAddress != null && prevIPAddress != null ) {
            ...
            return thisIPAddress.equals(prevIPAddress);
        }
        ...
        if( thisDNSName != null && prevDNSName != null ) {
            // only allow the exactly match
            return thisDNSName.equals(prevDNSName);
        }
