...
/**
 * Abstract base class for SSLSocketImpl. Its purpose is to house code with no
 * SSL related logic (or no logic at all). This makes SSLSocketImpl shorter and
 * easier to read. It contains a few constants and static methods plus
 * overridden java.net.Socket methods.
 *
 * Methods are defined final to ensure that they are not accidentally overridden
 * in SSLSocketImpl.
 *
...
abstract class BaseSSLSocketImpl extends SSLSocket {
    ...
    /*
     * Normally "self" is "this" ... but not when this connection is layered
     * over a preexisting socket. If we're using an existing socket, we delegate
     * some actions to it. Else, we delegate instead to "super". This is
     * important to ensure that we don't recurse infinitely ... e.g. close()
     * calling itself, or doing I/O in terms of our own streams.
     */
    final Socket self;
     ...
     * implementations (MS IIS and others) don't do that. The property below
     * controls whether we accept that or treat it as an error.
     *
    ...
    final static boolean requireCloseNotify = Debug.getBooleanProperty(
            PROP_NAME, false);
    ...
    /**
     * The semantics of shutdownInput is not supported in TLS 1.0 spec. Thus
     * when the method is called on an SSL socket, an
    ...
    /**
     * The semantics of shutdownOutput is not supported in TLS 1.0 spec. Thus
     * when the method is called on an SSL socket, an
     * UnsupportedOperationException will be thrown.
     *
     * @throws UnsupportedOperationException
    ...
    /**
     * Ensures that the SSL connection is closed down as cleanly as possible, in
     * case the application forgets to do so. This allows SSL connections to be
     * implicitly reclaimed, rather than forcing them to be explicitly reclaimed
     * at the penalty of prematurly killing SSL sessions.
     */
    ...
    /**
     * Returns true if the Nagle optimization is disabled. This relates to
     * low-level buffering of TCP traffic, delaying the traffic to promote
     * better throughput.
     *
     ...
     * 
     * @see java.net.Socket#sendUrgentData At this point, there seems to be no
     *      specific requirement to support this for an SSLSocket. An
     *      implementation can be provided if a need arises in future.
     */
     ...
     * discarded.
     * 
     * @see java.net.Socket#setOOBInline Setting OOBInline does not have any
     *      effect on SSLSocket, since currently we don't support sending urgent
     *      data.
     */
