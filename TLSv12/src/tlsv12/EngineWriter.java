...
final class EngineWriter {
    ...
    /*
     * Outgoing handshake Data waiting for a ride is stored here. Normal
     * application data is written directly into the outbound buffer, but
     * handshake data can be written out at any time, so we have buffer it
     * somewhere.
     * 
     * When wrap is called, we first check to see if there is any data waiting,
     * then if we're in a data transfer state, we try to write app data.
     * 
    ...
    /*
     * Upper levels assured us we had room for at least one packet of data. As
     * per the SSLEngine spec, we only return one SSL packets worth of data.
     */
    private HandshakeStatus getOutboundData(ByteBuffer dstBB) {
        ...
        /*
         * If we have more data in the queue, it's either a finished message, or
         * an indication that we need to call wrap again.
         */
    ...
    /*
     * Properly orders the output of the data written to the wrap call. This is
     * only handshake data, application data goes through the other writeRecord.
     */
    synchronized void writeRecord(EngineOutputRecord outputRecord,
        ...
        /*
         * Did our handshakers notify that we just sent the Finished message?
         * 
     ...
     */
    private static void dumpPacket(EngineArgs ea, boolean hsData) {
        try {
            ...
            System.out.println("[Raw write" + (hsData ? "" : " (bb)")
                    + "]: length = " + bb.remaining());
    ...
    /*
     * Properly orders the output of the data written to the wrap call. Only app
     * data goes through here, handshake data goes through the other
     * writeRecord.
     * 
     ...
     */
    synchronized HandshakeStatus writeRecord(EngineOutputRecord outputRecord,
            EngineArgs ea, MAC writeMAC, CipherBox writeCipher) throws IOException {

        /*
         * If we have data ready to go, output this first before trying to
         * consume app data.
         */
        if( hasOutboundDataInternal() ) {
            ...
            if( debug != null && Debug.isOn("packet") ) {
                 ...
                 * OutputRecord.write(OutputStream), but let's actually output
                 * when it's actually output by the SSLEngine.
                 */
                dumpPacket(ea, true);
            }
        ...
        /*
         * If we are closed, no more app data can be output. Only existing
         * handshake data (above) can be obtained.
         */
        if( outboundClosed ) {
        ...
        /*
         * No way new outbound handshake data got here if we're locked properly.
         * 
     ...
     * outputRecord.write() above. We already know this writer can accept more
     * data (outboundClosed == false), and the closure is sync'd.
     */
    ...
    /*
     * This is for the really rare case that someone is writing from the
     * *InputRecord* before we know what to do with it.
     */
