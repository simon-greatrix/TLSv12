...
/**
 * Encapsulates an SSL session ID. SSL Session IDs are not reused by servers
 * during the lifetime of any sessions it created. Sessions may be used by many
 * connections, either concurrently (for example, two connections to a web
 * server at the same time) or sequentially (over as long a time period as is
 * allowed by a given server).
 *
