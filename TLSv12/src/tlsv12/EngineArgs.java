...
/*
 * A multi-purpose class which handles all of the SSLEngine arguments. It
 * validates arguments, checks for RO conditions, does space calculations,
 * performs scatter/gather, etc.
 * 
...
class EngineArgs {
    ...
    /*
     * The initial pos/limit conditions. This is useful because we can quickly
     * calculate the amount consumed/produced in successful operations, or
     * easily return the buffers to their pre-error conditions.
     */
    private int netPos;
    ...
    /*
     * The main initialization method for the arguments. Most of them are pretty
     * obvious as to what they do.
     * 
     * Since we're already iterating over appData array for validity checking,
     * we also keep track of how much remainging space is available. Info is
     * used in both unwrap (to see if there is enough space available in the
     * destination), and in wrap (to determine how much more we can copy into
     * the outgoing data buffer.
     */
    private void init(ByteBuffer netData, ByteBuffer[] appData, int offset,
            int len) {
        ...
        for(int i = offset;i < offset + len;i++) {
            ...
            /*
             * If we're unwrapping, then check to make sure our destination
             * bufffers are writable.
             */
        ...
        /*
         * Ok, looks like we have a good set of args, let's store the rest of
         * this stuff.
         */
    ...
    /*
     * Given spaceLeft bytes to transfer, gather up that much data from the
     * appData buffers (starting at offset in the array), and transfer it into
     * the netData buffer.
     * 
    ...
    /*
     * Calculate the bytesConsumed/byteProduced. Aren't you glad we saved this
     * off earlier?
     */
    int deltaNet() {
    ...
    /*
     * In the case of Exception, we want to reset the positions to appear as
     * though no data has been consumed or produced.
     * 
     * Currently, this method is only called as we are preparing to fail out,
     * and thus we don't need to actually recalculate appRemaining. If that
     * assumption changes, that variable should be updated here.
     */
    void resetPos() {
    ...
    /*
     * We are doing lots of ByteBuffer manipulations, in which case we need to
     * make sure that the limits get set back correctly. This is one of the last
     * things to get done before returning to the user.
     */
    void resetLim() {
