...
/**
 * Abstraction for the SSL/TLS hash of all handshake messages that is maintained
 * to verify the integrity of the negotiation. Internally, it consists of an MD5
 * and an SHA1 digest. They are used in the client and server finished messages
 * and in certificate verify messages (if sent).
 *
 * This class transparently deals with cloneable and non-cloneable digests.
 *
 * This class now supports TLS 1.2 also. The key difference for TLS 1.2 is that
 * you cannot determine the hash algorithms for CertificateVerify at a early
 * stage. On the other hand, it's simpler than TLS 1.1 (and earlier) that there
 * is no messy MD5+SHA1 digests.
 *
 * You need to obey these conventions when using this class:
 *
 * 1. protocolDetermined(version) should be called when the negotiated protocol
 ...
 * all protocolDetermined(), setCertificateVerifyAlg() and setFinishedAlg() have
 * been called and the version is TLS 1.2. If a CertificateVerify message is to
 * be used, call setCertificateVerifyAlg() with the hash algorithm as the
 ...
 * </pre>
 * 
 * This call can be made once you are certain that this message will never be
 * used.
 */
final class HandshakeHash {
    ...
    /**
     * Create a new HandshakeHash. needCertificateVerify indicates whether a
     * hash for the certificate verify message is required. The argument algs is
     * a set of all possible hash algorithms that might be used in TLS 1.2. If
     * the caller is sure that TLS 1.2 won't be used or no CertificateVerify
     ...
     * digest clones that can be obtained. Digests that have already been cloned
     * and are gone remain gone.
     */
    void reset() {
        if( version != -1 ) {
     ...
     * based in info in ServerHello. Can be called multiple times.
     */
    ...
    /**
     * Specifies the hash algorithm used in CertificateVerify. Can be called
     * multiple times.
     */
    ...
    /**
     * Calculates the hash in the CertificateVerify. Must be called right after
     * setCertificateVerifyAlg()
     */
    /*
     * byte[] getCertificateVerifyHash() { throw new
     * Error("Do not call getCertificateVerifyHash()"); }
     */

    /**
     * Calculates the hash in Finished. Must be called after setFinishedAlg().
     * This method can be called twice, for Finished messages of the server side
...
/**
 * A wrapper for MessageDigests that simulates cloning of non-cloneable digests.
 * It uses the standard MessageDigest API and therefore can be used
 ...
 * construction time. This is achieved by internally maintaining n digests in
 * parallel. Consequently, it is only 1/n-th times as fast as the original
 * digest.
 *
 * Example: MessageDigest md = CloneableDigest.getDigest("SHA", 2);
 * md.update(data1); MessageDigest md2 = (MessageDigest)md.clone();
 * md2.update(data2); byte[] d1 = md2.digest(); // digest of data1 || data2
 * md.update(data3); byte[] d2 = md.digest(); // digest of data1 || data3
 *
...
final class CloneableDigest extends MessageDigest implements Cloneable {
    ...
    /**
     * The individual MessageDigests. Initially, all elements are non-null. When
     * clone() is called, the non-null element with the maximum index is
