...
final class SignatureAndHashAlgorithm {
    ...
    // messages.
    static SignatureAndHashAlgorithm valueOf(int hash, int signature,
            int sequence) {
        hash &= 0xFF;
        signature &= 0xFF;
        ...
        if( signAlg == null ) {
            ...
            signAlg = new SignatureAndHashAlgorithm("Unknown (hash:0x"
                    + Integer.toString(hash, 16) + ", signature:0x"
                    + Integer.toString(signature, 16) + ")", id, sequence);
        }
    ...
    private static void supports(HashAlgorithm hash,
        ...
        SignatureAndHashAlgorithm pair = new SignatureAndHashAlgorithm(hash,
                signature, algorithm, priority);
    ...
    static SignatureAndHashAlgorithm getPreferableAlgorithm(
        ...
        return SignatureAndHashAlgorithm.getPreferableAlgorithm(algorithms,
                expected, null);
    }
    ...
    static SignatureAndHashAlgorithm getPreferableAlgorithm(
        ...
        if( signingKey != null
            ...
            /*
             * RSA keys of 512 bits have been shown to be practically breakable,
             * it does not make much sense to use the strong hash algorithm for
             * keys whose key size less than 512 bits. So it is not necessary to
             * caculate the required max digest length exactly.
             * 
             * If key size is greater than or equals to 768, there is no max
             * digest length limitation in currect implementation.
             * 
             * If key size is greater than or equals to 512, but less than 768,
             * the digest length should be less than or equal to 32 bytes.
             * 
             * If key size is less than 512, the digest length should be less
             * than or equal to 20 bytes.
             */
    ...
    static enum HashAlgorithm {
        UNDEFINED("undefined", "", -1, -1), NONE("none", "NONE", 0, -1), MD5(
                "md5", "MD5", 1, 16), SHA1("sha1", "SHA-1", 2, 20), SHA224(
                "sha224", "SHA-224", 3, 28), SHA256("sha256", "SHA-256", 4, 32), SHA384(
                "sha384", "SHA-384", 5, 48), SHA512("sha512", "SHA-512", 6, 64);
    ...
    static {
        ...
        synchronized (supportedMap) {
            ...
            supports(HashAlgorithm.MD5, SignatureAlgorithm.RSA, "MD5withRSA",
                    --p);
            supports(HashAlgorithm.SHA1, SignatureAlgorithm.DSA, "SHA1withDSA",
                    --p);
            supports(HashAlgorithm.SHA1, SignatureAlgorithm.RSA, "SHA1withRSA",
                    --p);
