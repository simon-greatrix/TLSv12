 ...
 * ClientHello and ServerHello messages. The extension mechanism and several
 * extensions are defined in RFC 3546. Additional extensions are defined in the
 * ECC RFC 4492.
 *
 * Currently, only the two ECC extensions are fully supported.
 *
 * The classes contained in this file are: . HelloExtensions: a List of
 * extensions as used in the client hello and server hello messages. .
 * ExtensionType: an enum style class for the extension type . HelloExtension:
 * abstract base class for all extensions. All subclasses must be immutable.
 *
 * . UnknownExtension: used to represent all parsed extensions that we do not
 * explicitly support. . ServerNameExtension: the server_name extension. .
    ...
    HelloExtensions(HandshakeInStream s) throws IOException {
        ...
        while( len > 0 ) {
            ...
            if( extType == ExtensionType.EXT_SERVER_NAME ) {
                ...
                extension = new SupportedEllipticPointFormatsExtension(s);
...
final class ExtensionType {
    ...
    final static ExtensionType EXT_SERVER_NAME = e(0x0000, "server_name"); // IANA
                                                                           // registry
                                                                           // value:
                                                                           // 0

    // extensions defined in RFC 4492 (ECC)
    final static ExtensionType EXT_ELLIPTIC_CURVES = e(0x000A,
            "elliptic_curves"); // IANA registry value: 10

    final static ExtensionType EXT_EC_POINT_FORMATS = e(0x000B,
            "ec_point_formats"); // IANA registry value: 11

    // extensions defined in RFC 5246
    final static ExtensionType EXT_SIGNATURE_ALGORITHMS = e(0x000D,
            "signature_algorithms"); // IANA registry value: 13

    // extensions defined in RFC 5746
    final static ExtensionType EXT_RENEGOTIATION_INFO = e(0xff01,
            "renegotiation_info"); // IANA registry value: 65281
}
 ...
 * 'virtual' servers at a single underlying network address, clients MAY include
 * an extension of type "server_name" in the (extended) client hello. The
 * "extension_data" field of this extension SHALL contain "ServerNameList"
 * where:
 * 
...
final class SupportedEllipticCurvesExtension extends HelloExtension {
    ...
    public String toString() {
        ...
        for(int curveId:curveIds) {
            ...
            if( oid != null ) {
                ECParameterSpec spec = ECUtils.getECParameterSpec(oid);
    ...
    static int getCurveIndex(ECParameterSpec params) {
        String oid = ECUtils.getNamedCurveOid(params);
        if( oid == null ) {
            return -1;
        }
    ...
    private final static String[] NAMED_CURVE_OID_TABLE = new String[] { null, // (0)
                                                                               // unused
...
final class SupportedEllipticPointFormatsExtension extends HelloExtension {
    ...
    static final HelloExtension DEFAULT = new SupportedEllipticPointFormatsExtension(
    ...
    SupportedEllipticPointFormatsExtension(HandshakeInStream s)
            throws IOException {
        super(ExtensionType.EXT_EC_POINT_FORMATS);
        formats = s.getBytes8();
 ...
 * cryptographic binding to the enclosing TLS connection (if any) for which the
 * renegotiation is being performed. The "extension data" field of this
 * extension contains a "RenegotiationInfo" structure:
 * 
...
final class RenegotiationInfoExtension extends HelloExtension {
    ...
    RenegotiationInfoExtension(byte[] clientVerifyData, byte[] serverVerifyData) {
        ...
        if( clientVerifyData.length != 0 ) {
            renegotiated_connection = new byte[clientVerifyData.length
...
/*
 * [RFC5246] The client uses the "signature_algorithms" extension to indicate to
 * the server which signature/hash algorithm pairs may be used in digital
 ...
 * 
 * struct { HashAlgorithm hash; SignatureAlgorithm signature; }
