 ...
 *
 * Application data is kept external to the InputRecord, but handshake data
 * (alert/change_cipher_spec/handshake) will be kept internally in the
...
final class EngineInputRecord extends InputRecord {
    ...
    /*
     * A dummy ByteBuffer we'll pass back even when the data is stored
     * internally. It'll never actually be used.
     */
    static private ByteBuffer tmpBB = ByteBuffer.allocate(0);

    /*
     * Flag to tell whether the last read/parsed data resides internal in the
     * ByteArrayInputStream, or in the external buffers.
     */
    ...
    /*
     * Check if there is enough inbound data in the ByteBuffer to make a inbound
     * packet. Look for both SSLv2 and SSLv3.
     * 
    ...
    int bytesInCompletePacket(ByteBuffer buf) throws SSLException {
        ...
        /*
         * SSLv2 length field is in bytes 0/1 SSLv3/TLS length field is in bytes
         * 3/4
         */
        if( buf.remaining() < 5 ) {
            return -1;
        ...
        /*
         * If we have already verified previous packets, we can ignore the
         * verifications steps, and jump right to the determination. Otherwise,
         * try one last hueristic to see if it's SSL/TLS.
         */
        if( formatVerified || (byteZero == ct_handshake)
                || (byteZero == ct_alert) ) {
            /*
             * Last sanity check that it's not a wild record
             */
            ProtocolVersion recordVersion = ProtocolVersion.valueOf(
                    buf.get(pos + 1), buf.get(pos + 2));
            ...
            /*
             * Reasonably sure this is a V3, disable further checks. We can't do
             * the same in the v2 check below, because read still needs to
             * parse/handle the v2 clientHello.
             */
            formatVerified = true;
             ...
             */
            len = ((buf.get(pos + 3) & 0xff) << 8) + (buf.get(pos + 4) & 0xff)
                    + headerSize;

        } else {
            /*
             * Must be SSLv2 or something unknown. Check if it's short (2 bytes)
             * or long (3) header.
             * 
    ...
    /*
     * Pass the data down if it's internally cached, otherwise do it here.
     * 
     * If internal data, data is decrypted internally.
     * 
     * If external data(app), return a new ByteBuffer with data to process.
     */
     ...
     * 
     * If internal data (hs, alert, ccs), the data is read and stored
     * internally.
     * 
     * If external data (app), return a new ByteBuffer which points to the data
     * to process.
     */
    ByteBuffer read(ByteBuffer srcBB) throws IOException {
        /*
         * Could have a src == null/dst == null check here, but that was already
         * checked by SSLEngine.unwrap before ever attempting to read.
         */

        /*
         * If we have anything besides application data, or if we haven't even
         * done the initial v2 verification, we send this down to be processed
         * by the underlying internal cache.
         */
        if( !formatVerified
        ...
        /*
         * It's really application data. How much to consume? Jump over the
         * header.
         */
