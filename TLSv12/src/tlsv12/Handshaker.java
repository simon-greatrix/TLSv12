...
import tlsv12.CipherSuite.BulkCipher;
import tlsv12.CipherSuite.KeyExchange;
import tlsv12.CipherSuite.MacAlg;
...
/**
 * Handshaker ... processes handshake records from an SSL V3.0 data stream,
 * handling all the details of the handshake protocol.
 *
 * Note that the real protocol work is done in two subclasses, the base class
 * just provides the control flow and key generation framework.
 *
...
abstract class Handshaker {
     ...
     * 
     * Active protocols is a subset of enabled protocols, and will contain only
     * those protocols that have vaild cipher suites enabled.
     */
     ...
     * 
     * If thrown is set, we need to propagate this back immediately on entry
     * into processMessage().
     * 
    ...
    private void init(SSLContextImpl context, ProtocolList enabledProtocols,
        ...
        if( debug != null && Debug.isOn("handshake") ) {
            System.out.println("Allow unsafe renegotiation: "
     ...
     * 
     * We could have also done it by extra classes and letting them override,
     * but this seemed much less involved.
     */
    ...
    String getHostAddressSE() {
        ...
        /*
         * This is for caching only, doesn't matter that's is really a hostname.
         * The main thing is that it doesn't do a reverse DNS lookup,
    ...
    /**
     * Set the active protocol version and propagate it to the SSLSocket and our
     * handshake streams. Called from ClientHandshaker and ServerHandshaker with
     ...
     * SSLSocketImpl/SSLEngineImpl.setEnabledProtocols() (if the handshake is
     * not yet in progress).
     */
     ...
     * SSLSocketImpl/SSLEngineImpl.setEnabledCipherSuites() (if the handshake is
     * not yet in progress).
     */
    ...
    Collection<SignatureAndHashAlgorithm> getLocalSupportedSignAlgs() {
        if( localSupportedSignAlgs == null ) {
            localSupportedSignAlgs = SignatureAndHashAlgorithm.getSupportedAlgorithms();
        }
    ...
    void setPeerSupportedSignAlgs(
        ...
        peerSupportedSignAlgs = new ArrayList<SignatureAndHashAlgorithm>(
                algorithms);
    }
    ...
    /**
     * Set cipherSuite and keyExchange to the given CipherSuite. Does not
     * perform any verification that this is a valid selection, this must be
    ...
    /**
     * Check if the given ciphersuite is enabled and available. Does not check
     * if the required server certificates are available.
     */
    ...
    /**
     * Select a protocol version from the list. Called from ServerHandshaker to
     * negotiate protocol version.
     *
     * Return the lower of the protocol version suggested in the clien hello and
     ...
     *
     * In TLS 1.1, many weak or vulnerable cipher suites were obsoleted, such as
     * TLS_RSA_EXPORT_WITH_RC4_40_MD5. The implementation MUST NOT negotiate
     * these cipher suites in TLS 1.1 or later mode.
     *
     * Therefore, when the active protocols only include TLS 1.1 or later, the
     * client cannot request to negotiate those obsoleted cipher suites. That
     * is, the obsoleted suites should not be included in the client hello. So
     * we need to create a subset of the enabled cipher suites, the active
     * cipher suites, which does not contain obsoleted cipher suites of the
     * minimum active protocol.
     *
            ...
            if( !(activeProtocols.collection().isEmpty())
                ...
                for(CipherSuite suite:enabledCipherSuites.collection()) {
                    if( suite.obsoleted > activeProtocols.min.v
                        ...
                        if( suite.obsoleted <= activeProtocols.min.v ) {
                            System.out.println("Ignoring obsoleted cipher suite: "
                                    + suite);
                        } else {
                            System.out.println("Ignoring unsupported cipher suite: "
                                    + suite);
                        }
                    }
                }
     ...
     * these cipher suites in TLS 1.1 or later mode.
     * 
     * For example, if "TLS_RSA_EXPORT_WITH_RC4_40_MD5" is the only enabled
     * cipher suite, the client cannot request TLS 1.1 or later, even though TLS
     * 1.1 or later is enabled. We need to create a subset of the enabled
     * protocols, called the active protocols, which contains protocols
     * appropriate to the list of enabled Ciphersuites.
     * 
    ...
    ProtocolList getActiveProtocols() {
        if( activeProtocols == null ) {
            ...
            ArrayList<ProtocolVersion> protocols = new ArrayList<ProtocolVersion>(
                    4);
            for(ProtocolVersion protocol:enabledProtocols.collection()) {
                ...
                if( !found && (debug != null) && Debug.isOn("handshake") ) {
                    System.out.println("No available cipher suite for "
                            + protocol);
                }
            }
    ...
    /**
     * As long as handshaking has not activated, we can change whether session
     * creations are allowed.
     *
     * Callers should do their own checking if handshaking has activated.
     */
    ...
    /*
     * Returns true iff the handshake sequence is done, so that this freshly
     * created session can become the current one.
     */
    boolean isDone() {
    ...
    /*
     * Returns the session which was created through this handshake sequence ...
    ...
    /*
     * This routine is fed SSL handshake records when they become available, and
     * processes messages found therein.
     */
    void process_record(InputRecord r, boolean expectingFinished) throws IOException {
        ...
        /*
         * Store the incoming handshake data, then see if we can now process any
         * completed handshake messages
         */
        input.incomingRecord(r);

        /*
         * We don't need to create a separate delegatable task for finished
         * messages.
         */
    ...
    /*
     * On input, we hash messages one at a time since servers may need to access
     * an intermediate hash to validate a CertificateVerify message.
     * 
     * Note that many handshake messages can come in one record (and often do,
     * to reduce network resource utilization), and one message can also require
     * multiple records (e.g. very large Certificate messages).
     */
    void processLoop() throws IOException {
        ...
        while( input.available() >= 4 ) {
            ...
            /*
             * See if we can read the handshake message header, and then the
             * entire handshake message. If not, wait till we can read and
            ...
            /*
             * Process the messsage. We require that processMessage() consumes
             * the entire message. In lieu of explicit error checks (how?!) we
             * assume that the data will look like garbage on
             * encoding/processing errors, and that other protocol code will
             * detect such errors.
             * 
             * Note that digesting is normally deferred till after the message
             * has been processed, though to process at least the client's
             * Finished message (i.e. send the server's) we need to acccelerate
             * that digesting.
             * 
             * Also, note that hello request messages are never hashed; that
             * includes the hello request header, too.
             */
    ...
    /*
     * Used to kickstart the negotiation ... either writing a ClientHello or a
     * HelloRequest as appropriate, whichever the subclass returns. NOP if
     * handshaking's already started.
     */
    ...
    /**
     * Both client and server modes can start handshaking; but the message they
     * send to do so is different.
     */
    abstract HandshakeMessage getKickstartMessage() throws SSLException;


    /*
     * Client and Server side protocols are each driven though this call, which
     * processes a single message and drives the appropriate side of the
     * protocol state machine (depending on the subclass).
     */
    abstract void processMessage(byte messageType, int messageLen) throws IOException;


    /*
     * Most alerts in the protocol relate to handshaking problems. Alerts are
     * detected as the connection reads data.
     */
    abstract void handshakeAlert(byte description) throws SSLProtocolException;


    /*
     * Sends a change cipher spec message and updates the write side cipher
     * state so that future messages use the just-negotiated spec.
     */
    void sendChangeCipherSpec(Finished mesg, boolean lastMessage) throws IOException {
        ...
        /*
         * The write cipher state is protected by the connection write lock so
         * we must grab it while making the change. We also make sure no writes
         * occur between sending the ChangeCipherSpec message, installing the
         * new cipher state, and sending the Finished message.
         * 
         * We already hold SSLEngine/SSLSocket "this" by virtue of this being
         * called from the readRecord code.
         */
        OutputRecord r;
        if( conn != null ) {
    ...
    /*
     * Single access point to key calculation logic. Given the pre-master secret
     * and the nonces from client and server, produce all the keying material to
     * be used.
     */
    void calculateKeys(SecretKey preMasterSecret, ProtocolVersion version) {
        SecretKey master = calculateMasterSecret(preMasterSecret);
    ...
    /*
     * Calculate the master secret from its various components. This is used for
     * key exchange by all cipher suites.
     * 
     * The master secret is the catenation of three MD5 hashes, each consisting
     * of the pre-master secret and a SHA1 hash. Those three SHA1 hashes are of
     * (different) constant strings, the pre-master secret, and the nonces
     * provided by the client and the server.
     */
    private SecretKey calculateMasterSecret(SecretKey preMasterSecret) {
        ...
        PRF prf = cipherSuite.prfAlg;

        String prfHashAlg = prf.getPRFHashAlg();
        int prfHashLength = prf.getPRFHashLength();
        int prfBlockSize = prf.getPRFBlockSize();

        Tls12MasterSecret.Spec spec = new Tls12MasterSecret.Spec(
                preMasterSecret, protocolVersion.major, protocolVersion.minor,
                clnt_random.random_bytes, svr_random.random_bytes, prfHashAlg,
                prfHashLength, prfBlockSize);

        try {
            return (new Tls12MasterSecret(spec)).generateKey();
    ...
    /*
     * Calculate the keys needed for this connection, once the session's master
     * secret has been calculated. Uses the master key and nonces; the amount of
     * keying material generated is a function of the cipher suite that's been
     * negotiated.
     * 
     * This gets called both on the "full handshake" (where we exchanged a
     * premaster secret and started a new session) as well as on the
     * "fast handshake" (where we just resumed a pre-existing session).
     */
    void calculateConnectionKeys(SecretKey masterKey) {
        /*
         * For both the read and write sides of the protocol, we use the master
         * to generate MAC secrets and cipher keying material. Block ciphers
         * need initialization vectors, which we also generate.
         * 
        ...
        Tls12Material.Spec spec = new Tls12Material.Spec(masterKey,
                protocolVersion.major, protocolVersion.minor,
                clnt_random.random_bytes, svr_random.random_bytes,
                cipher.algorithm, cipher.keySize, expandedKeySize,
                cipher.ivSize, hashSize, prfHashAlg, prfHashLength,
                prfBlockSize);

        try {
            Tls12Material.KeyMaterial keySpec = (new Tls12Material(spec)).generateKey();
    ...
    /**
     * Return whether or not the Handshaker has derived session keys for this
     * handshake. This is used for determining readiness to process an incoming
    ...
    /**
     * Throw an SSLException with the specified message and cause. Shorthand
     * until a new SSLException constructor is added. This method never returns.
     */
     ...
     * 
     * We are currently implementing this as a single delegator, may try for
     * parallel tasks later. Client Authentication could benefit from this,
     * where ClientKeyExchange/CertificateVerify could be carried out in
     * parallel.
     */
    ...
    /*
     * The previous caller failed for some reason, report back the Exception. We
     * won't worry about Error's.
     * 
     * Locked by SSLEngine.this.
     */
    void checkThrown() throws SSLException {
        synchronized (thrownLock) {
            if( thrown != null ) {
                ...
                /*
                 * See what the underlying type of exception is. We should throw
                 * the same thing. Chain thrown to the new exception.
                 */
                Exception e = thrown;
                thrown = null;
                ...
                if( e instanceof RuntimeException ) {
                    ...
                    throw (SSLPeerUnverifiedException) new SSLPeerUnverifiedException(
                            msg).initCause(e);
                } else if( e instanceof SSLProtocolException ) {
                    throw (SSLProtocolException) new SSLProtocolException(msg).initCause(e);
                } else {
                    /*
                     * If it's SSLException or any other Exception, we'll wrap
                     * it in an SSLException.
                     */
