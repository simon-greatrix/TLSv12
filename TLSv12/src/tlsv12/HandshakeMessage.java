...
import tlsv12.CipherSuite.KeyExchange;
import tlsv12.CipherSuite.PRF;
...
/**
 * Many data structures are involved in the handshake messages. These classes
 * are used as structures, with public data members. They are not visible
 * outside the SSL package.
 *
 * Handshake messages all have a common header format, and they are all encoded
 * in a "handshake data" SSL record substream. The base class here
 * (HandshakeMessage) provides a common framework and records the SSL record
 * type of the particular handshake message.
 *
 * This file contains subclasses for all the basic handshake messages. All
 * handshake messages know how to encode and decode themselves on SSL streams;
 * this facilitates using the same code on SSL client and server sides, although
 * they don't send and receive the same messages.
 *
 * Messages also know how to print themselves, which is quite handy for
 * debugging. They always identify their type, and can optionally dump all of
 * their content.
 *
...
public abstract class HandshakeMessage {
    ...
    /**
     * Utility method to convert a BigInteger to a byte array in unsigned format
     * as needed in the handshake messages. BigInteger uses 2's complement
     * format, i.e. it prepends an extra zero if the MSB is set. We remove that.
     */
    ...
    /*
     * SSL 3.0 MAC padding constants. Also used by CertificateVerify and
     * Finished during the handshake.
     */
    ...
    /*
     * Write a handshake message on the (handshake) output stream. This is just
     * a four byte header followed by the data.
     * 
     * NOTE that huge messages -- notably, ones with huge cert chains -- are
     * handled correctly.
     */
    ...
    /*
     * Subclasses implement these methods so those kinds of messages can be
     * emitted. Base class delegates to subclass.
     */
     ...
     * 
     * By RFC2246:7.4.1.2 it's explicitly anticipated that this message will
     * have more data added at the end ... e.g. what CAs the client trusts.
     * Until we know how to parse it, we will just read what we know about, and
     * let our caller handle the jumps over unknown data.
     */
    static final class ClientHello extends HandshakeMessage {
        ...
        void addSignatureAlgorithmsExtension(
            ...
            HelloExtension signatureAlgorithm = new SignatureAlgorithmsExtension(
                    algorithms);
        ...
        int messageLength() {
            /*
             * Add fixed size parts of each field... version + random + session
             * + cipher + compress
             */
            return (2 + 32 + 1 + 2 + 1 + sessionId.length() /*
                                                             * ... + variable
                                                             * parts
                                                             */
                    + (cipherSuites.size() * 2) + compression_methods.length)
                    + extensions.length();
        }
     ...
     * 
     * Each end of a connection may need to pass its certificate chain to the
     * other end. Such chains are intended to validate an identity with
     * reference to some certifying authority. Examples include companies like
     * Verisign, or financial institutions. There's some control over the
     * certifying authorities which are sent.
     * 
        ...
        int messageLength() {
            if( encodedChain == null ) {
                ...
                try {
                    ...
                    encodedChain = null;
                    throw new RuntimeException("Could not encode certificates",
                            e);
                }
            }
     ...
     * 
     * The primary exceptions are when a server sends an encryption-capable RSA
     * public key in its cert, to be used with RSA (or RSA_export) key exchange;
     * and when a server sends its Diffie-Hellman cert. Those kinds of key
     * exchange do not require a ServerKeyExchange message.
     * 
     * Key exchange can be viewed as having three modes, which are explicit for
     * the Diffie-Hellman flavors and poorly specified for RSA ones:
     * 
     * - "Ephemeral" keys. Here, a "temporary" key is allocated by the server,
     * and signed. Diffie-Hellman keys signed using RSA or DSS are ephemeral
     * (DHE flavor). RSA keys get used to do the same thing, to cut the key size
     * down to 512 bits (export restrictions) or for signing-only RSA
     * certificates.
     * 
     * - Anonymity. Here no server certificate is sent, only the public key of
     * the server. This case is subject to man-in-the-middle attacks. This can
     * be done with Diffie-Hellman keys (DH_anon) or with RSA keys, but is only
     * used in SSLv3 for DH_anon.
     * 
     * - "Normal" case. Here a server certificate is sent, and the public key
     * there is used directly in exchanging the premaster secret. For example,
     * Diffie-Hellman "DH" flavor, and any RSA flavor with only 512 bit keys.
     * 
     * If a server certificate is sent, there is no anonymity. However, when a
     * certificate is sent, ephemeral keys may still be used to exchange the
     * premaster secret. That's how RSA_EXPORT often works, as well as how the
     * DHE_* flavors work.
     */
    ...
    /*
     * Using Diffie-Hellman algorithm for key exchange. All we really need to do
     * is securely get Diffie-Hellman keys (using the same P, G parameters) to
     * our peer, then we automatically have a shared secret without need to
     * exchange any more data. (D-H only solutions, such as SKIP, could
     * eliminate key exchange negotiations and get faster connection setup. But
     * they still need a signature algorithm like DSS/DSA to support the trusted
     * distribution of keys without relying on unscalable physical key
     * distribution systems.)
     * 
     * This class supports several DH-based key exchange algorithms, though
     * perhaps eventually each deserves its own class. Notably, this has basic
     * support for DH_anon and its DHE_DSS and DHE_RSA signed variants.
     */
    static final class DH_ServerKeyExchange extends ServerKeyExchange {
        // Fix message encoding, see 4348279
        private final static boolean dhKeyExchangeFix = Debug.getBooleanProperty(
                "com.sun.net.ssl.dhKeyExchangeFix", true);
        ...
        // protocol version being established using this ServerKeyExchange
        // message
        ProtocolVersion protocolVersion;

        // the preferable signature algorithm used by this ServerKeyExchange
        // message
        private SignatureAndHashAlgorithm preferableSignatureAlgorithm;


        /*
         * Construct from initialized DH key object, for DH_anon key exchange.
         */
        ...
        /*
         * Construct from initialized DH key object and the key associated with
         * the cert chain which was sent ... for DHE_DSS and DHE_RSA key
         * exchange. (Constructor called by server.)
         */
        DH_ServerKeyExchange(DHCrypt obj, PrivateKey key, byte clntNonce[],
            ...
            Signature sig = Signature.getInstance(signAlgorithm.getAlgorithmName());
        ...
        /*
         * Construct a DH_ServerKeyExchange message from an input stream, as if
         * sent from server to client for use with DH_anon key exchange
         */
        DH_ServerKeyExchange(HandshakeInStream input,
            ...
            KeyUtil.validate(new DHPublicKeySpec(new BigInteger(1, dh_Ys),
                    new BigInteger(1, dh_p), new BigInteger(1, dh_g)));
        ...
        /*
         * Construct a DH_ServerKeyExchange message from an input stream and a
         * certificate, as if sent from server to client for use with DHE_DSS or
         * DHE_RSA key exchange. (Called by client.)
         */
        DH_ServerKeyExchange(HandshakeInStream input, PublicKey publicKey,
            ...
            if( protocolVersion.v >= ProtocolVersion.TLS12.v ) {
                ...
                preferableSignatureAlgorithm = SignatureAndHashAlgorithm.valueOf(
                        hash, signature, 0);
            ...
            Signature sig = Signature.getInstance(preferableSignatureAlgorithm.getAlgorithmName());
    ...
    /*
     * ECDH server key exchange message. Sent by the server for ECDHE and
     * ECDH_anon ciphersuites to communicate its ephemeral public key (including
    ...
    static final class ECDH_ServerKeyExchange extends ServerKeyExchange {
        ...
        ECDH_ServerKeyExchange(ECDHCrypt obj, PrivateKey privateKey,
            ...
            pointBytes = ECUtils.encodePoint(point, params.getCurve());
        ...
        ECDH_ServerKeyExchange(HandshakeInStream input, PublicKey signingKey,
            ...
            if( curveType == CURVE_NAMED_CURVE ) {
                ...
                if( curveOid == null ) {
                    throw new SSLHandshakeException("Unknown named curve: "
                            + curveId);
                }
                parameters = ECUtils.getECParameterSpec(curveOid);
                if( parameters == null ) {
                    throw new SSLHandshakeException("Unsupported curve: "
                            + curveOid);
                }
            } else {
                throw new SSLHandshakeException("Unsupported ECCurveType: "
                        + curveType);
            }
            pointBytes = input.getBytes8();

            ECPoint point = ECUtils.decodePoint(pointBytes,
                    parameters.getCurve());
            ECKeyFactory factory = new ECKeyFactory();
            publicKey = (ECPublicKey) factory.generatePublic(new ECPublicKeySpec(
                    point, parameters));
    ...
    static final class DistinguishedName {
        ...
        /*
         * DER encoded distinguished name. TLS requires that its not longer than
         * 65535 bytes.
         */
        byte name[];
     ...
     * 
     * Authenticated servers may ask clients to authenticate themselves in turn,
     * using this message.
     * 
     * Prior to TLS 1.2, the structure of the message is defined as: struct {
     * ClientCertificateType certificate_types<1..2^8-1>; DistinguishedName
     * certificate_authorities<0..2^16-1>; } CertificateRequest;
     * 
     * In TLS 1.2, the structure is changed to: struct { ClientCertificateType
     * certificate_types<1..2^8-1>; SignatureAndHashAlgorithm
     * supported_signature_algorithms<2^16-1>; DistinguishedName
     * certificate_authorities<0..2^16-1>; } CertificateRequest;
     */
    static final class CertificateRequest extends HandshakeMessage {
        ...
        private final static byte[] TYPES_ECC = { cct_rsa_sign, cct_dss_sign,
                cct_ecdsa_sign };
        ...
        // protocol version being established using this CertificateRequest
        // message
        ProtocolVersion protocolVersion;
        ...
        CertificateRequest(X509Certificate ca[], KeyExchange keyExchange,
            ...
            // We only request ECDSA client auth if we have ECC crypto
            // available.
            ...
            if( protocolVersion.v >= ProtocolVersion.TLS12.v ) {
                ...
                algorithmsLen = SignatureAndHashAlgorithm.sizeInRecord()
                        * algorithms.size();
            } else {
        ...
        CertificateRequest(HandshakeInStream input,
            ...
            if( protocolVersion.v >= ProtocolVersion.TLS12.v ) {
                ...
                while( remains > 1 ) { // needs at least two bytes
                    ...
                    SignatureAndHashAlgorithm algorithm = SignatureAndHashAlgorithm.valueOf(
                            hash, signature, ++sequence);
     ...
     * client certificate request) it sends this message to flag that it's done
     * that part of the handshake.
     */
    ...
    static final class CertificateVerify extends HandshakeMessage {
        ...
        // the preferable signature algorithm used by this CertificateVerify
        // message
        ...
        /*
         * Verify a certificate verify message. Return the result of
         * verification, if there is a problem throw a GeneralSecurityException.
         */
         ...
         * signature algorithm and protocol version so that the object is ready
         ...
         * all preceding handshake messages. Used by the Finished class as well.
         */
        private static void updateDigest(MessageDigest md, byte[] pad1,
                byte[] pad2, SecretKey masterSecret) {
            ...
            byte[] keyBytes = "RAW".equals(masterSecret.getFormat()) ? masterSecret.getEncoded()
                    : null;
            if( keyBytes != null ) {
                md.update(keyBytes);
            } else {
        ...
        // Note that this will prevent the Spi classes from being GC'd. We
        // assume
        // that is not a problem.
        private final static Map<Class<?>, Object> methodCache = new ConcurrentHashMap<Class<?>, Object>();
        ...
        private static void digestKey(MessageDigest md, SecretKey key) {
            try {
                ...
                if( r == null ) {
                    try {
                        r = clazz.getDeclaredMethod("implUpdate",
                                SecretKey.class);
                ...
                throw new RuntimeException("Could not obtain encoded key and "
     ...
     * Both protocols define this handshake message slightly differently. This
     * class supports both formats.
     * 
    ...
    static final class Finished extends HandshakeMessage {
        ...
        /*
         * Contents of the finished message ("checksum"). For TLS, it is 12
         * bytes long, for SSLv3 36 bytes.
         */
        ...
        Finished(ProtocolVersion protocolVersion, HandshakeInStream input,
            ...
            this.protocolVersion = protocolVersion;
            this.cipherSuite = cipherSuite;
            int msgLen = (protocolVersion.v >= ProtocolVersion.TLS10.v) ? 12
                    : 36;
         ...
         * according to a given set of inputs. This is used to ensure that both
         * client and server are fully in sync, and that the handshake
         ...
         */
        private byte[] getFinished(HandshakeHash handshakeHash, int sender,
                SecretKey masterKey) {
            byte[] sslLabel;
            String tlsLabel;
            if( sender == CLIENT ) {
            ...
            if( protocolVersion.v >= ProtocolVersion.TLS10.v ) {
                ...
                try {
                    ...
                    /*
                     * RFC 5246/7.4.9 says that finished messages can be
                     * ciphersuite-specific in both length/PRF hash algorithm.
                     * If we ever run across a different length, this call will
                     * need to be updated.
                     */
                    Tls12Prf.Spec spec = new Tls12Prf.Spec(masterKey, tlsLabel,
                            seed, 12, prfHashAlg, prfHashLength, prfBlockSize);

                    SecretKey prfKey = (new Tls12Prf(spec)).generateKey();
        ...
        /*
         * Update the MessageDigest for SSLv3 finished message calculation. The
         * digest must already have been updated with all preceding handshake
         * messages. This operation is almost identical to the certificate
         * verify hash, reuse that code.
         */
