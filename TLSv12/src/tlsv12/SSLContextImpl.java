...
public abstract class SSLContextImpl extends SSLContextSpi {
    ...
    protected void engineInit(KeyManager[] km, TrustManager[] tm,
        ...
        /*
         * The initial delay of seeding the random number generator could be
         * long enough to cause the initial handshake on our first connection to
         * timeout and fail. Make sure it is primed and ready by getting some
         * initial output from it.
         */
    ...
    private static X509TrustManager chooseTrustManager(TrustManager[] tm) {
    ...
    private static X509ExtendedKeyManager chooseKeyManager(KeyManager[] kms) {
        for(int i = 0;kms != null && i < kms.length;i++) {
            ...
            if( debug != null && Debug.isOn("sslctx") ) {
                System.out.println("X509KeyManager passed to "
    ...
    ProtocolList getSuportedProtocolList() {
        if( supportedProtocolList == null ) {
            supportedProtocolList = new ProtocolList(
                    getSupportedSSLParams().getProtocols());
        }
     ...
     * See: SSLSocket/SSLEngine.setEnabledProtocols()
     */
    ...
    private static CipherSuiteList getApplicableCipherSuiteList(
        ...
        if( !(protocols.collection().isEmpty())
            ...
            for(CipherSuite suite:allowedCipherSuites) {
                ...
                if( suite.isAvailable() && suite.obsoleted > protocols.min.v
                        && suite.supported <= protocols.max.v ) {
                    suites.add(suite);
                } else if( debug != null && Debug.isOn("sslctx")
                        && Debug.isOn("verbose") ) {
                    if( suite.obsoleted <= protocols.min.v ) {
                        ...
                        System.out.println("Ignoring unavailable cipher suite: "
                                + suite);
                    }
                }
            }
     ...
     * internally, there is no need to clear the cache and calling this method
     * has no effect.
     *
     ...
     * 
     * SSL/TLS protocols specify the forward compatibility and version roll-back
     * attack protections, however, a number of SSL/TLS server vendors did not
     * implement these aspects properly, and some current SSL/TLS servers may
     * refuse to talk to a TLS 1.1 or later client.
     * 
     * Considering above interoperability issues, SunJSSE will not set TLS 1.1
     * and TLS 1.2 as the enabled protocols for client by default.
     * 
     * For SSL/TLS servers, there is no such interoperability issues as SSL/TLS
     * clients. In SunJSSE, TLS 1.1 or later version will be the enabled
     * protocols for server by default.
     * 
     * We may change the behavior when popular TLS/SSL vendors support TLS
     * forward compatibility properly.
     * 
     * SSLv2Hello is no longer necessary. This interoperability option was put
     * in place in the late 90's when SSLv3/TLS1.0 were relatively new and there
     * were a fair number of SSLv2-only servers deployed. Because of the
     * security issues in SSLv2, it is rarely (if ever) used, as deployments
     * should now be using SSLv3 and TLSv1.
     * 
     * Considering the issues of SSLv2Hello, we should not enable SSLv2Hello by
     * default. Applications still can use it by enabling SSLv2Hello with the
     * series of setEnabledProtocols APIs.
     */

    /*
     * The conservative SSLContext implementation for TLS, SSL, SSLv3 and TLS10
     * algorithm.
     * 
     ...
     * @see SSLContext
     */
    static class ConservativeSSLContext extends SSLContextImpl {
        // parameters
        ...
        static {
            ...
            supportedSSLParams.setProtocols(new String[] { ProtocolVersion.TLS12.name });

            serverCandidates = new ProtocolVersion[] { ProtocolVersion.TLS12 };

            clientCandidates = new ProtocolVersion[] { ProtocolVersion.TLS12 };

            defaultClientSSLParams = new SSLParameters();
            defaultClientSSLParams.setProtocols(getAvailableProtocols(clientCandidates));

            defaultServerSSLParams = new SSLParameters();
            defaultServerSSLParams.setProtocols(getAvailableProtocols(serverCandidates));
        }
    ...
    public static final class DefaultSSLContext extends ConservativeSSLContext {
        ...
        static synchronized SSLContextImpl getDefaultImpl() throws Exception {
            if( defaultImpl == null ) {
                defaultImpl = new DefaultSSLContext();
            }
            return defaultImpl;
        }
...
final class DummyX509TrustManager implements X509TrustManager {
     ...
     * validated and is trusted for client SSL authentication. If not, it throws
     * an exception.
     */
    @Override
     ...
     * Given the partial or complete certificate chain provided by the peer,
     * build a certificate path to a trusted root and return if it can be
     * validated and is trusted for server SSL authentication. If not, it throws
     * an exception.
     */
    @Override
    ...
    /*
     * Return an array of issuer certificates which are trusted for
...
final class DummyX509KeyManager extends X509ExtendedKeyManager {
    ...
    /*
     * Choose an alias to authenticate the client side of an engine given the
     * public key type and the list of certificate issuer authorities recognized
     * by the peer (if any).
     */
    ...
    /*
     * Choose an alias to authenticate the server side of an engine given the
     * public key type and the list of certificate issuer authorities recognized
     * by the peer (if any).
     */
